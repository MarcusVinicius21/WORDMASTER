    ...
    return (
      <section className="py-16 bg-gray-50">
        <div className="container mx-auto px-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">{[...Array(4)].map((_, i) => (<PropertyCardSkeleton key={i} />))}</div>
        </div>
      </section>
    );
  }
...
const PropertyCard = ({ listing, category }) => {
    const getPropertyImage = (listing, category) => {
      ...
      const renderSpecifications = () => {
        ...
        switch (category) {
          case 'mansoes':
            if (listing.guests) specs.push(<span key="guests" className="flex items-center"><Users className="w-4 h-4 mr-1.5" />{listing.guests} Hóspedes</span>);
            if (listing.bedrooms) specs.push(<span key="bedrooms" className="flex items-center"><Bed className="w-4 h-4 mr-1.5" />{listing.bedrooms} Quartos</span>);
            if (listing.bathrooms) specs.push(<span key="bathrooms" className="flex items-center"><Bath className="w-4 h-4 mr-1.5" />{listing.bathrooms} Banheiros</span>);
            if (listing.area_m2) specs.push(<span key="area" className="flex items-center"><Maximize className="w-4 h-4 mr-1.5" />{listing.area_m2}m²</span>);
            break;
          case 'lanchas':
            if (listing.guests) specs.push(<span key="guests" className="flex items-center"><Users className="w-4 h-4 mr-1.5" />{listing.guests} Passageiros</span>);
            if (listing.boat_length) specs.push(<span key="length" className="flex items-center"><Anchor className="w-4 h-4 mr-1.5" />{listing.boat_length} pés</span>);
            if (listing.bedrooms) specs.push(<span key="cabins" className="flex items-center"><Bed className="w-4 h-4 mr-1.5" />{listing.bedrooms} Cabines</span>);
            if (listing.boat_year) specs.push(<span key="year" className="flex items-center"><Calendar className="w-4 h-4 mr-1.5" />{listing.boat_year}</span>);
            break;
          case 'escuna':
            if (listing.guests) specs.push(<span key="guests" className="flex items-center"><Users className="w-4 h-4 mr-1.5" />{listing.guests} Pessoas</span>);
            if (listing.duration) specs.push(<span key="duration" className="flex items-center"><Clock className="w-4 h-4 mr-1.5" />{listing.duration}</span>);
            break;
          case 'taxi-aereo':
            if (listing.guests) specs.push(<span key="guests" className="flex items-center"><Users className="w-4 h-4 mr-1.5" />{listing.guests} Passageiros</span>);
            if (listing.vehicle_type) { const vehicleIcon = <Plane className="w-4 h-4 mr-1.5" />; const vehicleText = {'helicopter': 'Helicóptero', 'bimotor': 'Bimotor', 'jet': 'Jato'}[listing.vehicle_type] || listing.vehicle_type; specs.push(<span key="vehicle" className="flex items-center">{vehicleIcon}{vehicleText}</span>); }
            if (listing.duration) specs.push(<span key="duration" className="flex items-center"><Clock className="w-4 h-4 mr-1.5" />{listing.duration}</span>);
            break;
          case 'transfer':
            if (listing.guests) specs.push(<span key="guests" className="flex items-center"><Users className="w-4 h-4 mr-1.5" />{listing.guests} Passageiros</span>);
            if (listing.vehicle_type) { const vehicleIcon = listing.vehicle_type === 'bus' ? <Bus className="w-4 h-4 mr-1.5" /> : <Car className="w-4 h-4 mr-1.5" />; const vehicleText = {'luxury_car': 'Carro de Luxo', 'van': 'Van', 'bus': 'Ônibus'}[listing.vehicle_type] || listing.vehicle_type; specs.push(<span key="vehicle" className="flex items-center">{vehicleIcon}{vehicleText}</span>); }
            if (listing.duration) specs.push(<span key="duration" className="flex items-center"><Clock className="w-4 h-4 mr-1.5" />{listing.duration}</span>);
            break;
          case 'buggy':
            if (listing.guests) specs.push(<span key="guests" className="flex items-center"><Users className="w-4 h-4 mr-1.5" />{listing.guests} Pessoas</span>);
            if (listing.vehicle_model) specs.push(<span key="model" className="flex items-center"><Car className="w-4 h-4 mr-1.5" />{listing.vehicle_model}</span>);
            if (listing.duration) specs.push(<span key="duration" className="flex items-center"><Clock className="w-4 h-4 mr-1.5" />{listing.duration}</span>);
            break;
          default:
            if (listing.guests) specs.push(<span key="guests" className="flex items-center"><Users className="w-4 h-4 mr-1.5" />{listing.guests} Pessoas</span>);
        ...
        <div className="bg-gradient-to-r from-gray-800 to-gray-900 rounded-2xl p-8 md:p-12 text-white flex flex-col md:flex-row items-center justify-between gap-8 shadow-2xl">
          ...
          <div className="flex-shrink-0">
            <a href={`https://wa.me/5521976860759?text=${encodeURIComponent("Olá! Gostaria de planejar minha experiência em Búzios.")}`} target="_blank" rel="noopener noreferrer">
               <Button size="lg" className="bg-brand-yellow text-gray-900 hover:bg-brand-yellow/90 rounded-full px-10 py-6 text-base font-semibold shadow-lg transition-transform hover:scale-105">Falar com um Especialista</Button>
            </a>
          </div>
        </div>
      </div>
