            ...
            return (
              <button 
                ...
                className={`w-full flex items-center space-x-3 px-4 py-3 rounded-lg transition-colors ${ 
                  currentPage === item.id ? 'bg-blue-600 text-white' : 'text-gray-300 hover:bg-gray-800 hover:text-white'
                }`}
              >
    ...
    const fetchStats = async () => {
      try {
        const [listingsRes, reviewsRes] = await Promise.all([
          fetch('/api/listings?active=false'), 
          fetch('/api/reviews?is_approved=false')
        ]);
        if (listingsRes.ok) {
          ...
          setStats(prev => ({ 
            ...
            onPromotion: listings.filter(l => l.is_featured).length 
          }));
        }
        if (reviewsRes.ok) {
      ...
      {loading ? (
        ...
        <div className="text-center py-12">
          ...
          <div className="flex justify-center space-x-2 flex-wrap gap-2">
            <Button onClick={() => { setEditingListing({ category: 'mansoes' }); setOpenModal(true) }} className="bg-blue-600 hover:bg-blue-700">
              <Plus className="w-4 h-4 mr-2" />Mansão
            </Button>
            <Button onClick={() => { setEditingListing({ category: 'lanchas' }); setOpenModal(true) }} className="bg-cyan-600 hover:bg-cyan-700">
              <Plus className="w-4 h-4 mr-2" />Lancha
            </Button>
            <Button onClick={() => { setEditingListing({ category: 'escuna' }); setOpenModal(true) }} className="bg-teal-600 hover:bg-teal-700">
              <Plus className="w-4 h-4 mr-2" />Escuna
            </Button>
          </div>
        </div>
      )}
...
const CreateListingModal = ({ onSave, editingListing, onClose }) => {
  const [formData, setFormData] = useState({
    category: '', title: '', subtitle: '', description: '', neighborhood: '', guests: '', bedrooms: '', 
    bathrooms: '', area_m2: '', base_price: '', price_label: '', is_featured: false,
          ...
          <>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div>
                ...
                <Input type="number" value={formData.guests} onChange={(e) => setFormData(p => ({ ...p, guests: e.target.value }))} />
              </div>
              <div>
                <label className="block text-sm font-medium mb-1">Quartos</label>
                <Input type="number" value={formData.bedrooms} onChange={(e) => setFormData(p => ({ ...p, bedrooms: e.target.value }))} />
              </div>
              <div>
                <label className="block text-sm font-medium mb-1">Banheiros</label>
                <Input type="number" value={formData.bathrooms} onChange={(e) => setFormData(p => ({ ...p, bathrooms: e.target.value }))} />
              </div>
              <div>
                <label className="block text-sm font-medium mb-1">Área (m²)</label>
                <Input type="number" value={formData.area_m2} onChange={(e) => setFormData(p => ({ ...p, area_m2: e.target.value }))} />
              </div>
            </div>
          </>
        );
        ...
        return (
          <>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <div>
                <label className="block text-sm font-medium mb-1">Tamanho (pés)</label>
                <Input type="number" value={formData.boat_length} onChange={(e) => setFormData(p => ({ ...p, boat_length: e.target.value }))} />
              </div>
              <div>
                <label className="block text-sm font-medium mb-1">Cabines</label>
                <Input type="number" value={formData.bedrooms} onChange={(e) => setFormData(p => ({ ...p, bedrooms: e.target.value }))} />
              </div>
              <div>
                <label className="block text-sm font-medium mb-1">Ano</label>
                <Input type="number" value={formData.boat_year} onChange={(e) => setFormData(p => ({ ...p, boat_year: e.target.value }))} />
              </div>
            </div>
          </>
        );
          ...
          <>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              ...
              <div>
                ...
                <Input value={formData.duration} onChange={(e) => setFormData(p => ({ ...p, duration: e.target.value }))} placeholder="Ex: 4 horas" />
              </div>
              <div>
                <label className="block text-sm font-medium mb-1">Tamanho (pés)</label>
                <Input type="number" value={formData.boat_length} onChange={(e) => setFormData(p => ({ ...p, boat_length: e.target.value }))} />
            ...
            <div className="flex items-center space-x-2">
              <input 
                ...
                onChange={(e) => setFormData(p => ({ ...p, includes_meal: e.target.checked }))} 
              />
              <label htmlFor="includes_meal" className="text-sm font-medium">Inclui refeição</label>
            </div>
          </>
        );
        ...
        return (
          <>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <div>
                <label className="block text-sm font-medium mb-1">Tipo de Aeronave</label>
              ...
              <div>
                ...
                <Input 
                  ...
                  onChange={(e) => setFormData(p => ({ ...p, vehicle_model: e.target.value }))} 
                  placeholder="Ex: Robinson 44" 
                />
              </div>
              <div>
                <label className="block text-sm font-medium mb-1">Duração</label>
      ...
      <form onSubmit={handleSubmit} className="space-y-6 pt-4">
        <div>
          <label className="block text-sm font-medium mb-1">
            Fotos {uploading && <span className="text-blue-600">(Fazendo upload...)</span>}
          </label>
          <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center">
            ...
            <label htmlFor="image-upload" className={`cursor-pointer ${uploading ? 'opacity-50' : ''}`}>
              ...
              <p className="text-lg font-medium text-gray-600 mb-2">
                {uploading ? 'Fazendo upload das imagens...' : 'Clique para adicionar múltiplas fotos'}
              </p>
              <p className="text-sm text-gray-500">Agora você pode adicionar quantas fotos quiser!</p>
            </label>
          </div>
          {uploadedImages.length > 0 && (
            <div className="mt-4">
              <p className="text-sm text-gray-600 mb-3">{uploadedImages.length} foto(s) adicionada(s)</p>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div>
              ...
              <Select required value={formData.category} onValueChange={(v) => setFormData(p => ({ ...p, category: v }))}>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="lanchas">Lancha</SelectItem>
                  <SelectItem value="escuna">Escuna</SelectItem>
                  <SelectItem value="taxi-aereo">Táxi Aéreo</SelectItem>
                  <SelectItem value="transfer">Transfer</SelectItem>
                  <SelectItem value="buggy">Buggy</SelectItem>
        ...
        <div>
          ...
          <Input 
            ...
            placeholder={`Ex: ${getCategoryTitle()} de luxo`} 
            required 
          />
        </div>
        <div>
          ...
          <Input 
            ...
            placeholder="Ex: Luxo e conforto" 
          />
        </div>
        <div>
        ...
        <div className="flex items-center space-x-2">
          ...
          />
          <label htmlFor="featured" className="text-sm font-medium">Marcar como destaque</label>
        </div>
